...
function matchJob(job: { skills: { name: string }[]; location: { name: string }; visaSupport: boolean }, userSkills: string[], japaneseLevel: string, experienceYears: number, targetCity: string) {
  ...
  return Math.min(100, skillScore + cityScore + visaScore + languageScore + experienceScore);
}
...
export default async function PersonalizedMatches() {
  ...
  const jobs = await db.job.findMany({ include: { company: true, location: true, category: true, skills: true }, orderBy: { createdAt: "desc" } });
  ...
  const rankedJobs = jobs.map((job) => ({ job, match: matchJob(job, userSkills, japaneseLevel, user.profile!.experienceYears, user.profile!.targetCity) })).sort((a, b) => b.match - a.match);
  ...
  return <main className="container py-16">
    ...
    <div className="mt-10 space-y-4">{rankedJobs.length ? rankedJobs.map(({ job, match }) => <Link href={`/jobs/${job.slug}`} className="card block p-6 hover:border-ocean" key={job.id}><div className="flex flex-col justify-between gap-4 md:flex-row"><div><p className="text-3xl font-bold text-ocean">{match}% <span className="text-sm font-normal text-slate-500">Match</span></p><h2 className="mt-1 text-xl font-bold">{job.title}</h2><p className="mt-1 text-sm text-slate-500">{job.company.name} · {job.location.name} · {job.category.name}</p></div><div className="text-sm text-slate-500"><p>{job.skills.length ? `${job.skills.map((skill) => skill.name).join(", ")}` : "Skills to be discussed"}</p><p className="mt-2 font-semibold text-ocean">{job.visaSupport ? "Visa support" : "Check visa fit"}</p></div></div><div className="mt-4 h-2 rounded-full bg-slate-100"><div style={{ width: `${match}%` }} className="h-2 rounded-full bg-ocean" /></div></Link>) : <p className="card p-6">No jobs are available yet. Check back soon.</p>}</div>
  </main>;
}

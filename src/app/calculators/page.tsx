...
const initialCosts: Costs = { rent: 90000, food: 40000, transportation: 15000, utilities: 12000, internet: 5000, entertainment: 20000 };
const costFields: { key: keyof Costs; label: string }[] = [
	...
	{ key: "internet", label: "Internet" }, { key: "entertainment", label: "Entertainment & other" },
];
		...
		<div className="mt-10 grid gap-6 md:grid-cols-[1fr_1fr]">
			<div className="card space-y-5 p-7">
				<label className="block font-semibold">Location
					<select value={location} onChange={(event) => setLocation(event.target.value)} className="mt-2 w-full rounded-lg border p-3"><option>Tokyo</option><option>Osaka</option><option>Fukuoka</option><option>Kyoto</option><option>Yokohama</option></select>
				</label>
